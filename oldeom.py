...
class EdgeOMatic:
    ...
    def __init__(self, ip: str, port: int):
        ...
        # Send stream command but don't strictly wait for a specific ack if firmware varies
        self.ws.send(msgspec.json.encode({"streamReadings": None}))

        # Safely fetch config
        self._config = self.send_and_recv_struct(
            self.ws, {"configList": None}, wait_for_name="configList", type=EdgeOMaticConfig
        )
